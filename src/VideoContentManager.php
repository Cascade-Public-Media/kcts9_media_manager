...
class VideoContentManager extends ApiContentManagerBase {
   ...
   */
  const LAST_UPDATE_KEY = 'kcts9_media_manager.video_content.last_update';

  /**
   * Video Content batch queue machine name.
   */
  const BATCH_QUEUE_NAME = 'kcts9_media_manager.queue.video_content.batch_queue';
  ...
  public static function getQueueName(): string {
    return 'kcts9_media_manager.queue.video_content';
  }
   ...
   *
   * @param string $guid
   *   GUID of the Asset to get.
   *
   * @return object|null
   *   Asset item data from the API or NULL.
   */
  public function getAsset(string $guid): ?stdClass {
    return $this->client->getAsset($guid);
  }
   ...
   * guideline maximum for _other_ data model object endpoint so that limit is
  ...
  public function updateQueue(DateTime $since = NULL): bool {
    ...
    // Rekey the array using Media Manager GUIDs.
    ...
    foreach ($batches as $batch) {
      ...
      foreach ($assets as $asset) {
        ...
        if ($updated_at > $since) {
          // Find the Asset's parent Show GUID.
          ...
          if (isset($parent_tree['show'])) {
            if (isset($shows[$parent_tree['show']])) {
              ...
              // GUID not found in $shows would potentially indicate some bug
              // with this code or with the API response. This should never
              // happen.
              $this->logger->error($this->t('Unexpected parent Show @show_guid
                for Asset @asset_guid .', [
                  '@show_guid' => $parent_tree['show'],
                  '@asset_guid' => $asset->id,
                ]));
            }
          }
            ...
            $this->logger->error($this->t('No parent Show found for Asset GUID
              @guid.', ['@guid' => $asset->id]));
          }
          continue;
        }
   ...
   *
   * @return array
   *   Ordered array keyed by parent object types and their Media Manager GUID
   *   values. E.g. --
   *
   * @code
   * <?php
  ...
  public function addOrUpdateVideoContent(
    ...
    $availabilities = [
      ...
      // (KCTS 9 in our case) and currently we do not use this functionality in
      // Media Manager.
  ...
  public static function getDateFieldPairs(): array {
    return [
      ...
      // Station is not currently considered because KCTS 9 does not limit
      // content to station members. Evaluating this field would also require
      // looking at the parent Show's audience stations.
      /*'station' => [
